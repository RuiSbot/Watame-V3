...
async function startGojoMdNx() {
    ...
    // Get Profile Picture Group
       ...
       GojoMdNx.send5ButImg(pea[0].id, `「 CHAT DESHABILITADO 」\n\nAHORA SOLO LOS ADMINISTRADORES PODRÁN ENVIAR MENSAJES !`, `🌸Watame🌸`, wm_fatih, [])
       } else if(pea[0].announce == false) {
       GojoMdNx.send5ButImg(pea[0].id, `「 CHAT HABILITADO 」\n\nAHORA TODOS PUEDEN ENVIAR MENSAJES !`, `🌸Watame🌸`, wm_fatih, [])
       } else if (pea[0].restrict == true) {
       GojoMdNx.send5ButImg(pea[0].id, `「 AJUSTES CAMBIADOS 」\n\nAHORA SOLO LOS ADMINISTRADORES PUEDEN CAMBIAR LA INFO DEL GRUPO !`, `🌸Watame🌸`, wm_fatih, [])
       } else if (pea[0].restrict == false) {
       GojoMdNx.send5ButImg(pea[0].id, `「 AJUSTES CAMBIADOS 」\n\nAHORA TODOS LOS PARTICIPANTES PUEDEN CAMBIAR LA INFO DEL GRUPO !`, `🌸Watame🌸`, wm_fatih, [])
       } else {
       GojoMdNx.send5ButImg(pea[0].id, `「 AJUSTES CAMBIADOS 」\n\nEL NOMBRE DEL GRUPO SE HA CAMBIADO A *${pea[0].subject}*`, `🌸Watame🌸`, wm_fatih, [])
     }
    })
